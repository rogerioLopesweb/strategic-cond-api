    ...
    try {
        ...
        if (uniRes.rows.length === 0) {
            return res.status(404).json({ 
                success: false, 
                message: "Unidade não encontrada no cadastro deste condomínio." 
            });
        }

        req.body.unidade_id = uniRes.rows[0].id;
        return vincularMoradorUnidade(req, res);
    } catch (error) {
        console.error('StrategicCond - Erro ao vincular por bloco:', error);
        res.status(500).json({ success: false, error: error.message });
    }
};
