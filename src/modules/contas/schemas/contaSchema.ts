...
export const contaIdParamSchema = z.object({
  id: z.string().uuid({ message: "ID da conta inválido." }).openapi({
...
export const createContaSchema = registry.register(
  "CreateContaInput",
  z.object({
    razao_social: z.string().min(1, "Razão social é obrigatória").openapi({
...
export const updateContaSchema = registry.register(
  ...
  z.object({
    ...
    status_conta: z.string().optional().openapi({
      example: "ativa",
      description: "Status da conta (ativa, suspensa, aguardando_configuracao)",
    }),
  }),
);
