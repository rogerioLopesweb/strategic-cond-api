...
// @ts-ignore
import { gerarLinkVisualizacao, uploadFoto } from "./storageService";
import {
  CreateUsuarioDTO,
  UpdateUsuarioDTO,
  ListUsuarioFilters,
...
s
  ...
  async cadastrarCompleto(dados: CreateUsuarioDTO) {
    ...
    try {
      ...
      if (dados.foto_base64) {
        ...
        if (pathRelativo) {
          urlFoto = await gerarLinkVisualizacao(pathRelativo);
          await client.query(
  ...
  async atualizarFoto(usuarioId: string, fotoBase64: string) {
    const nomeArquivo = `foto-${usuarioId}-${Date.now()}`;
    const pathRelativo = await uploadFoto(fotoBase64, nomeArquivo);

    if (!pathRelativo) throw new Error("Falha no upload do arquivo.");

    const urlFoto = await gerarLinkVisualizacao(pathRelativo);
